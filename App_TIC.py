    ...
    try:
        ...
        while True:
            ...
            if ingresou:
                ...
                print(Horas[1])
                codgenerado = envio_codigo(ingresou[4])
...
def reconocimiento_facial(nombre_usuario):
    ...
    cap = inicializar_captura()  # Captura
...
def solicita_peticion(sql, valor, tipo):
...
def solicita_ingreso(valor):
    ...
    valores = (valor,)
    return solicita_peticion(sql, valores, 1)
...
def registro_ingreso(nombre_usuario, horaI, Ingreso1, Ingreso2, Ingreso3):
    ...
    solicita_peticion(sql, valores, 2)
...
def register():
    if request.method == "POST":
        ...
        sql = "INSERT INTO registro_usuarios (nombre_usuario, contrasena,codigo_rfid,id_telegram) VALUES (%s, %s, %s, %s)"
        valores = (nombre_usuario, contrasena, codigoRfid, idtelegram)
        solicita_peticion(sql, valores, 1)
        # Redirige a la página de registro facial con el nombre de usuario
        return redirect(url_for('facial_register', nombre_usuario=nombre_usuario))
    else:
...
def login():
    if request.method == "POST":
        ...
        usuario_encontrado = solicita_peticion(sql, valores, 1)
...
def control():
    ...
    usuarios = solicita_peticion(sql, "None", 0)
